...
// Detail 游戏报表详情
func (that *GameController) Detail(ctx *fasthttp.RequestCtx) {

	ty := ctx.QueryArgs().GetUintOrZero("ty")               //1-游戏报表,2-游戏报表场馆
	...
	data, err := model.GameReportUser(ty, flag, dateFlag, timeFlag, startDate, endDate, id, page, pageSize)
	if err != nil {
		helper.Print(ctx, false, err.Error())
		return
	}
